...
def _align_face(gray: np.ndarray, box):
    # Always cast to native Python ints/floats
    try:
        x, y, w, h = [int(v) for v in box]
    except Exception:
        return gray  # if box cannot be parsed, bail out

    # Guard: invalid sizes → no-op
    if w <= 0 or h <= 0:
        return gray
    ...
    # Eye-based alignment (best-effort; never crash)
    try:
        ...
        # Any OpenCV type/shape issue → skip alignment
        pass
...
def _preprocess_face_for_model(bgr: np.ndarray, box):
    ...
    try:
        ...
        # Return a neutral tensor that won't crash the model
        return np.zeros((1, 96, 96, 3), dtype="float32")
    ...
    # Grayscale → (best-effort) align → resize → RGB → [0,1]
    gray = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)
    # Align in local coords; protect with try/except in case cascade is missing
    try:
